...
function createCacheMongoRepository<M extends Model, C>(clazz: ClassType<M>) {
    return class extends MongoRepository<M, C>(clazz) {
...
export default <M extends Model, C>(clazz: ClassType<M>) => {
    return createCacheMongoRepository<M, C>(clazz);
};
